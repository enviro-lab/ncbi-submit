...
class NCBI:
    ...
    def _add_primer_schemes(self,df):
        ...
        df = df.rename(columns={"primer_scheme":"Primer Scheme","primer scheme":"Primer Scheme",})
        print(df)
        exit()
        ...
        if self.primer_map:
            ...
            if "Primer Scheme" in primers.columns:
                ...
                # print(df.columns)
                # print(df)
                raise NotImplementedError("Must check this code (in `_add_primer_schemes()`) before submitting anything with primer scheme col in primer_map")
                return df
    ...
    def getBioBrojectLink(accession=None):
        ...
        if accession:
            ...
            raise NotImplementedError("`getBioBrojectLink(None)` - no accession given - code not ready for this")
            # return f'<PrimaryId db="BioProject">{accession}</PrimaryId>'
